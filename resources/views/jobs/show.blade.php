    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
        ...
        <section class="md:col-span-3">
            ...
            <div class="container mx-auto p-4">
                ...
                @auth
                    ...
                    @if ($job->user_id !== auth()->id())
                        ...
                        @endif
                    @else
                        {{-- Show message for job owner --}}
                        <div class="mb-4 mt-6 rounded border border-blue-400 bg-blue-100 px-4 py-3 text-blue-700">
            ...
            <div class="mt-6 rounded-lg bg-white p-6 shadow-md">
                <div id="map" {{-- class="z-[1] h-[400px] w-full" --}}></div>
            </div>
        </section>
        ...
        <aside class="rounded-lg bg-white p-3 shadow-md">
            ...
            @else
                <form method="POST" action="{{ route('bookmarks.store', $job->id) }}" class="mt-10">
                    @csrf
                    <button
                        class="{{ $bookmarked ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600' }} flex w-full items-center justify-center rounded-full px-4 py-2 font-bold text-white">
                        <i class="fas fa-bookmark mr-3"></i> {{ $bookmarked ? 'Remove Bookmark' : 'Bookmark Listing' }}
                    </button>
                </form>
            @endguest
